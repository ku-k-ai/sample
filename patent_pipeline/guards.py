...
def validate_final(result: dict[str, Any], handoff: dict[str, Any]) -> None:
    """最終段が資格を書き換えたり、不整合を抱えて候補を確定することを拒否。"""
    _require(isinstance(result, dict), "最終結果はオブジェクトが必要です")
    eligible = handoff["選択可能候補ID"]
    evaluations = _index(result.get("候補評価"), "候補ID")
    _require(set(evaluations) == set(eligible), "最終比較の候補集合が選択資格と一致しません")
    decision = result.get("最終判定")
    _require(isinstance(decision, str) and (decision == "自信無" or decision in eligible), "選択不可候補を最終段で復活できません")
    issues = result.get("前段不整合")
    _require(isinstance(issues, list), "前段不整合は配列が必要です")
    _require(not issues or decision == "自信無", "前段不整合がある場合は自信無が必要です")
    for row in evaluations.values():
        _require(row.get("主な届け先との関係") in {"主担当", "関連候補", "判断根拠不足"}, "未知の主担当関係です")
    primary = [cid for cid, row in evaluations.items() if row["主な届け先との関係"] == "主担当"]
    _require(primary == ([] if decision == "自信無" else [decision]), "主担当の件数又はIDが最終判定と一致しません")
